...
"""
Notice that we previously interpolate IV curve along tau-dimension for tau not observed on that day between a close observed tau pair (tau_L, tau_H). 

Now I want to iterate date and plot a 3-by-2 subfigure for each day. 

For each date, the first row is the Q and IV curves and raw IVs with tau >= 27 that has observed IV on that date. The second row is the Q and IV curve for tau = 27. The 3rd row is the Q and IV curve and raw IVs for tau <= 27. 
"""
...
def plot_tau_comparison(date_str, Q_data_dir, iv_surface_dir, df_obs, grid_full, tol, target_tau=27, save_dir="."):
    ...
    for idx, tau in enumerate(taus):
        ...
        # Note: If no raw IV data is observed exactly at the tau of interest, obs_for_tau will be empty.
...
def process_all_dates(Q_data_dir, iv_surface_dir, obs_data_path, grid_full, tol, target_tau=27, save_dir="."):
    """
    Process all available dates for which Q density files exist for the target_tau.
    
    This function extracts dates from the Q density files in the folder corresponding to target_tau
    and then creates the 3×2 comparison plot for each date.
    """
    ...
    for date_str in dates:
        plot_tau_comparison(date_str, Q_data_dir, iv_surface_dir, df_obs, grid_full, tol, target_tau, save_dir)
...
# Filter observed IV data as needed.
df_obs = df_obs[(df_obs['moneyness'] >= -1) & (df_obs['moneyness'] <= 1)]
df_obs = df_obs[(df_obs['tau'] >= 3) & (df_obs['tau'] <= 120)]
df_obs['IV'] = df_obs['IV'] / 100  

# Process all dates to generate and save the 3x2 plots.
process_all_dates(Q_data_dir, iv_surface_dir, obs_data_path, grid_full, tol, target_tau, save_dir)
